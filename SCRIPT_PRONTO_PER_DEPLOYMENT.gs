...
// VERSIONE FINALE CONFIGURATA E PRONTA
// ============================================
...
// ============================================
// FUNZIONE GET: Controlla disponibilità E Salva dati
// ============================================
function doGet(e) {
  try {
    Logger.log('doGet chiamato');
    Logger.log('Parametri: ' + JSON.stringify(e.parameter));
    
    const action = e.parameter.action;
    
    // Se action=save, salva i dati
    if (action === 'save') {
      Logger.log('Azione: SAVE');
      
      const data = {
        festeggiato: e.parameter.festeggiato || '',
        anni: e.parameter.anni || '',
        cliente: e.parameter.cliente || '',
        telefono: e.parameter.telefono || '',
        email: e.parameter.email || '',
        data_festa: e.parameter.data_festa || '',
        ora: e.parameter.ora || '',
        partecipanti: e.parameter.partecipanti || '',
        menu: e.parameter.menu || '',
        patatine: e.parameter.patatine || 'No',
        torta: e.parameter.torta || '',
        scritta_torta: e.parameter.scritta_torta || '',
        foto_torta: e.parameter.foto_torta || 'No',
        durata: e.parameter.durata || '',
        altre_richieste: e.parameter.altre_richieste || '',
        allergie_dichiarate: e.parameter.allergie_dichiarate || 'No',
        gdpr: e.parameter.gdpr || 'No',
        marketing: e.parameter.marketing || 'No',
        totale: e.parameter.totale || '0'
      };
    ...
    // Altrimenti, controlla disponibilità (comportamento originale)
    const date = e.parameter.date;
    const time = e.parameter.time;
    
    Logger.log('Azione: CHECK AVAILABILITY - Date: ' + date + ', Time: ' + time);
    ...
    Logger.log('Disponibilità: ' + available + ' (Eventi trovati: ' + birthdayEvents.length + ')');
...
function saveToSheet(data) {
  ...
  if (!sheet) {
    Logger.log('Foglio "' + SHEET_NAME + '" non trovato, creazione in corso...');
    ...
    Logger.log('Foglio creato con intestazioni');
  }
  ...
  Logger.log('Riga aggiunta al foglio, numero riga: ' + lastRow);
